...
const SectionTitle = ({ children, icon: Icon }: { children: React.ReactNode, icon: any }) => (
  <div className="flex flex-col gap-2 mb-10">
    ...
    <div className="flex items-center gap-3">
      ...
      <h2 className="text-4xl font-serif text-text-main tracking-tight italic">Exploring {children}</h2>
    </div>
  </div>
);
    ...
    <div className="min-h-screen font-sans bg-bg-main text-text-main overflow-x-hidden">
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 bg-bg-main/80 backdrop-blur-xl border-b border-border">
        <div className="max-w-7xl mx-auto px-8 lg:px-12 h-24 flex items-center justify-between">
          <div className="flex flex-col group cursor-pointer">
            <span className="text-2xl font-serif tracking-tighter leading-none text-accent group-hover:italic transition-all">AFAQ.</span>
            <span className="label-caps mt-1 text-text-muted">Full Stack Dev</span>
          </div>
          
          <div className="hidden md:flex items-center gap-10 text-[11px] uppercase tracking-widest text-text-muted">
            <a href="#about" className="hover:text-accent transition-colors">Biography</a>
            <a href="#experience" className="hover:text-accent transition-colors">Curation</a>
          ...
          <div className="lg:col-span-8 flex flex-col justify-center">
            ...
            >
              <div className="flex items-center gap-4 mb-8">
                ...
                <span className="label-caps text-text-muted">Currently based in {data.personal.location}</span>
              </div>
              ...
              <p className="max-w-lg text-lg text-text-muted leading-relaxed font-light mb-12">
                A {data.personal.role} focused on architecting immersive digital ecosystems, specialized in high-frequency CRM and enterprise systems with <span className="text-text-main font-medium italic underline decoration-accent/30 tracking-tight">uncompromising elegance.</span>
              </p>
            ...
            >
              <div className="absolute inset-0 bg-bg-card rounded-t-[160px] border-subtle overflow-hidden relative group">
                ...
                <div className="absolute bottom-8 left-0 right-0 text-center">
                  <span className="label-caps !opacity-40">{data.personal.name} ©</span>
                </div>
              </div>
              ...
              <div className="absolute -right-12 top-16 glass-theme-border bg-bg-main p-5 border-subtle border-accent/30">
                <div className="text-3xl font-serif leading-none italic">2+</div>
                <div className="label-caps !text-[7px] mt-1 text-text-muted">Years Tenure</div>
              </div>
      ...
      <section className="grid grid-cols-2 lg:grid-cols-4 border-b border-border">
        {[
          ...
          <div key={i} className="p-10 border-r border-border last:border-r-0 flex flex-col gap-2 hover:bg-bg-card transition-colors group">
            <span className="label-caps !text-text-muted group-hover:text-accent transition-colors">{stat.label}</span>
            <span className="text-md font-serif italic text-text-main truncate">{stat.val}</span>
          </div>
        ))}
      </section>

      {/* About Section */}
      <section id="about" className="py-32 px-8 lg:px-12 border-b border-border">
        <div className="max-w-7xl mx-auto grid lg:grid-cols-12 gap-20">
          ...
          <div className="lg:col-span-5">
            <h3 className="text-5xl font-serif leading-tight mb-8">
              ...
              <span className="text-accent underline decoration-accent/20">Scalability.</span>
            </h3>
            <p className="text-text-muted text-lg font-light leading-relaxed">
              Based in the digital frontier, I specialize in translating complex business requirements into elegant, high-performance web solutions.
            </p>
          </div>
          <div className="lg:col-span-7 flex flex-col justify-end">
            ...
            <div className="grid sm:grid-cols-2 gap-8">
              {["CRM Architect", "Scalable Systems", "Interface Mastery", "Performance Tuning"].map((tag) => (
                <div key={tag} className="flex items-center gap-4 group">
                  ...
                  <span className="label-caps text-text-muted group-hover:text-text-main">{tag}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      {/* Skills Section */}
      <section id="skills" className="py-32 px-8 lg:px-12 bg-[#0F0F0F] border-b border-border">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-0 border border-border">
            {data.skills.map((skillGroup, idx) => (
              <div key={idx} className="p-12 border border-border bg-bg-main/20 hover:bg-bg-main transition-all group">
                <div className="flex items-center justify-between mb-8">
                   <h3 className="font-serif italic text-2xl text-text-main">{skillGroup.category}</h3>
                   <div className="p-3 border-subtle rounded-full text-text-muted group-hover:text-accent group-hover:border-accent transition-all">
                    ...
                    {skillGroup.category === "Databases" && <Database size={18} />}
                    {skillGroup.category === "Cloud & DevOps" && <Cloud size={18} />}
                    {skillGroup.category === "Mobile & Other" && <Globe size={18} />}
                  </div>
                </div>
      ...
      {/* Experience Section */}
      <section id="experience" className="py-32 px-8 lg:px-12 border-b border-border">
              ...
              >
                ...
                <div className="grid lg:grid-cols-12 gap-10">
                  <div className="lg:col-span-4">
                    <span className="label-caps !text-accent mb-2 block">{exp.period}</span>
                    <h3 className="text-3xl font-serif italic text-text-main mb-2 tracking-tight">{exp.role}</h3>
                    <div className="text-text-muted text-sm font-light tracking-widest uppercase">@ {exp.company}</div>
                  </div>
                  <div className="lg:col-span-8">
                    <ul className="space-y-4">
                      {exp.achievements.map((item, i) => (
                        <li key={i} className="flex gap-4 text-text-muted font-light leading-relaxed text-md">
                          <span className="text-accent mt-[6px] shrink-0">•</span>
                          {item}
                        </li>
                      ))}
                    </ul>
        ...
        <div className="max-w-7xl mx-auto">
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-10 mb-20">
            <div className="max-w-2xl">
              ...
              <p className="text-text-muted text-lg font-light leading-relaxed">
                A selection of architectural achievements in enterprise platforms and specialized systems.
              </p>
            </div>
            <a href="#" className="label-caps !text-accent border-b border-accent/30 pb-2 hover:border-accent transition-all group">
              Exploration <span className="inline-block group-hover:translate-x-1 transition-transform">→</span>
            </a>
          </div>

          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-0 border border-border">
            {data.projects.map((project, idx) => (
              <div key={idx} className="group flex flex-col border border-border bg-bg-main/50 hover:bg-bg-main transition-all duration-500 overflow-hidden min-h-[400px]">
                <div className="p-10 flex flex-col h-full">
                  <div className="flex justify-between items-start mb-10">
                    <span className="label-caps tracking-[0.4em] opacity-30 group-hover:opacity-100 group-hover:text-accent transition-all italic text-[8px]">0{idx + 1} / WORK</span>
                    <a 
                      href={project.link} 
                      target="_blank" 
                  ...
                  <div className="flex flex-wrap gap-x-4 gap-y-2 mt-auto pt-6 border-t border-border/30">
                    {project.tech.map((t) => (
                      <span key={t} className="label-caps !text-[7px] text-text-muted group-hover:text-accent transition-colors">
                        {t}
                      </span>
                    ))}
                  </div>
        ...
        <div className="max-w-7xl mx-auto grid lg:grid-cols-12 gap-20">
          <div className="lg:col-span-8 p-12 border-subtle bg-bg-card relative overflow-hidden group">
            ...
            <div className="relative z-10">
              <h3 className="text-4xl font-serif italic mb-4">{data.education.degree}</h3>
              <div className="text-accent text-xl font-light mb-4">{data.education.institution}</div>
              <div className="flex items-center gap-6">
                <span className="label-caps text-text-muted">{data.education.period}</span>
                <span className="w-10 h-[1px] bg-border" />
                <span className="text-accent italic font-serif text-lg">{data.education.metrics}</span>
              </div>
            </div>
          </div>
          ...
          <div className="lg:col-span-4 flex flex-col gap-8">
            ...
            <div className="space-y-6">
              {data.languages.map((lang) => (
                <div key={lang} className="flex items-end justify-between border-b border-border pb-4 group">
                  <span className="text-2xl font-serif italic text-text-main group-hover:text-accent transition-colors">{lang}</span>
                  <span className="label-caps !text-text-muted opacity-50 group-hover:opacity-100">Native / Proficient</span>
                </div>
              ))}
            </div>
          </div>
      ...
      <footer className="py-24 px-8 lg:px-12 border-t border-border bg-bg-card">
        <div className="max-w-7xl mx-auto grid lg:grid-cols-2 gap-16 items-center">
          <div>
            <span className="text-5xl font-serif italic tracking-tighter text-accent leading-none">Let's Craft.</span>
            <p className="text-text-muted text-lg font-light mt-6 max-w-md">
              Available for strategic collaborations and architectural inquiries. Let's discuss your next digital legacy.
            </p>
            <div className="flex gap-10 mt-10">
               <a href={`mailto:${data.personal.email}`} className="group flex flex-col">
                <span className="label-caps !text-text-muted group-hover:text-accent transition-colors mb-2">Electronic Mail</span>
                <span className="text-xl font-serif italic border-b border-accent/20 group-hover:border-accent group-hover:text-accent transition-all">{data.personal.email}</span>
              </a>
              <a href={data.personal.linkedin} className="group flex flex-col">
                <span className="label-caps !text-text-muted group-hover:text-accent transition-colors mb-2">Social Nexus</span>
                <span className="text-xl font-serif italic border-b border-accent/20 group-hover:border-accent group-hover:text-accent transition-all">Connect on LinkedIn</span>
              </a>
            </div>
          </div>
          ...
          <div className="flex flex-col items-end justify-end">
            <div className="text-right group cursor-pointer">
              <span className="text-[120px] font-serif font-bold tracking-tighter leading-none opacity-5 group-hover:opacity-10 transition-opacity">MA.</span>
            </div>
            <p className="label-caps !text-text-muted mt-8">© 2024 Muhammad Afaq Ahmad — All Rights Reserved</p>
          </div>
        </div>
      </footer>
    </div>
